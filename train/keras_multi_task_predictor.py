...
flat5 = Flatten()(conv5)
flat6 = Dense(512, activation='relu')(flat5)

flat6_dropout = Dropout(0.5)(flat6)
drag_out = Dense(1, activation='linear')(flat6_dropout)

model = Model(inputs=[inputs], outputs=[conv10, drag_out])
...
# train model
model.fit(train_geometries, [train_steady_flows, train_drag_vectors],
          batch_size=batch_size,
          epochs=epochs,
          verbose=1,
          validation_data=(test_geometries, [test_steady_flows, test_drag_vectors]))

# eval model on test set
[predicted_steady_flow, predicted_drag_vector] = model.predict(test_geometries, batch_size=batch_size)
for i in xrange(predicted_steady_flow.shape[0]):
  ...
  plt.imshow(velocity_image)
  plt.show()
  # print predicted drag
  print("geometry in plot")
  print("true drag is     : " + str(test_drag_vectors[i]))
  print("predicted drag is: " + str(predicted_drag_vector[i]))
